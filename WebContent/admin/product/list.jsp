...
<script type="text/javascript">
			function addProduct(){
				window.location.href = "${pageContext.request.contextPath}/adminCategortList";
			}
			...
			function editProduct(pid){
				location.href = "${ pageContext.request.contextPath }/adminEditUI?pid="+pid;
			}
	...
	<form id="Form1" name="Form1"
		...
		商品类别：<select name="cid">
					<c:forEach items="${categoryList }" var="categoryList">
						<option value="${categoryList.cid }">${categoryList.cname }</option>
					</c:forEach>
				</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<td class="ta_01" align="center" bgColor="#f5fafe">
						<table cellspacing="0" cellpadding="1" rules="all"
							...
							<c:forEach items="${ProductList }" var="pro" varStatus="vs">
								...
								<td style="CURSOR: hand; HEIGHT: 22px" align="center"
									width="17%">
									<c:if test="${pro.is_hot == 0}">是</c:if>
									<c:if test="${pro.is_hot != 0}">否</c:if>
									</td>
								<td align="center" style="HEIGHT: 22px"><a
									href="javascript:void(0);" onclick="editProduct('${pro.pid}')">
										<img
